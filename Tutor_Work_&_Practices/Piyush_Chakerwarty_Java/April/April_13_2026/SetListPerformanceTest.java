...
// Purpose: Compares performance of different Collections (HashSet, LinkedHashSet, TreeSet, ArrayList, LinkedList).
public class SetListPerformanceTest {
    static final int N = 50000; // 50,000 elements for testing

    public static void main (String[] args) {
        // Add numbers 0, 1, 2, ..., N - 1 to the array list
        List<Integer> list = new ArrayList<>();
        for (int i = 0; i < N; i++) 
            list.add(i); // adds numbers 0 to 49,999 to ArrayList
        Collections.shuffle(list); // Shuffle the array list to stimulate real data

        // Create a hash set, and test its performance
        Collection<Integer> set1 = new HashSet<>(list); // Creates HashSet from the list (removes duplicates automatically)
        System.out.println("Member test time for hash set is " + 
            getTestTime(set1) + " milliseconds");
            // getTestTime(): Calls .contains() 50,000 times with random numbers (0 to 99,999)
        System.out.println("Remove element time for hash set is " + 
            getRemoveTime(set1) + " milliseconds");
            // getRemoveTime(): Calls .remove(i) for i = 0 to 49,999
        ...
        System.out.println("Remove element time for tre set is " + 
    ...
    // Why different results?
        // HashSet: O(1) average for contains/remove - fastest
        // TreeSet: O(log n) - slower due to tree balancing
        // ArrayList: O(n) for contains (linear search) - very slow
        // LinkedList: O(n) for contains - also slow
}
